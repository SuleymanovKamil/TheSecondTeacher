        ...
        ScrollView(showsIndicators: false) {
            VStack(alignment: .center, spacing: 20) {
                ...
                VStack(spacing: 10) {
                    ...
                    Text("Коран состоит сур, а суры из аятов. Традиционные знаки препинания, как точка или запятая, в Коране не используются. Вместо них на конец аята указывает специальный символ ۝ (либо другой символ в виде цветка и т.п. ٭), он и используется как знак остановки чтения.\n\nЕсли вы делаете паузу в чтении на ая те, или если вам просто нужно набрать воздух, тогда последняя гласная в слове или стоит танвин, заменяются сукуном. Если же на конце слова стоит алиф, а перед ним танвин Ан, то нун на конце танвина не читается, а алиф тянется. Например: يُسْرًا читается как Юсраа")
                        .fixedSize(horizontal: false, vertical: true)
                }
                .padding(.bottom, 20)
